...
class VisualOdometry:
    ...
    def pose_estimation(self, features_coor):
        ...
        f_l_prev, f_l_cur = features_coor[:,:2], features_coor[:,4:6]

        # Inverse camera model to get 3D point clouds for previous and current frame

        landmarks_prev = self.stereo_inverse(f_l_prev,f_r_prev)
        landmarks_cur = self.stereo_inverse(f_l_cur,f_r_cur)

        inlier_range = 2
        trials = 300
        f_r_prev_filt, f_r_cur_filt, p_ja_filt, p_jb_filt = self.RANSAC(landmarks_prev,landmarks_cur,f_r_prev,f_r_cur,inlier_range,trials)
        C, r, status = self.align_points(p_ja_filt, p_jb_filt)
    ...
    # New function Added to Starter Code
    def stereo_inverse(self, f_l, f_r):
        ...
        for i in range(f_l.shape[0]):
            landmark = (b/diff_u[i])*np.array([[0.5*sum_u[i]-cx],[(fx/fy)*(0.5*sum_v[i]-cy)],[fx]])
            landmarks.append(landmark)
        
        landmark_array = np.concatenate(landmarks,axis=1)
        return landmark_array
    
    # RANSAC Algorithm
    # New function Added to Starter Code
    def RANSAC(self, p_ja, p_jb, f_r_prev, f_r_curr, inlier_range, trials):
        ...
        for i in range(trials):
            while 1:
                random_inds = np.random.choice(np.arange(p_ja.shape[1]), size=3, replace=False)

                p_ja_rand = np.concatenate([p_ja[:,random_inds[0]],p_ja[:,random_inds[1]],p_ja[:,random_inds[2]]]).reshape(3,3).T
                p_jb_rand = np.concatenate([p_jb[:,random_inds[0]],p_jb[:,random_inds[1]],p_jb[:,random_inds[2]]]).reshape(3,3).T

                C, r, status = self.align_points(p_ja_rand,p_jb_rand)
                if status == True:
                    break
                else:
                    print("Fail")

            # Test if inlier and outlier
            model_p_jb = C@p_ja + r
            residuals = p_jb - model_p_jb
            errors = np.linalg.norm(residuals,axis=0)

            inlier_indices = errors<inlier_range
            num_inliers = np.sum(inlier_indices)
            if num_inliers > max_inliers:
                f_r_prev_best = f_r_prev[inlier_indices,:]
                f_r_cur_best = f_r_curr[inlier_indices,:]
                p_ja_best = p_ja[:,inlier_indices]
                p_jb_best = p_ja[:,inlier_indices]

        return f_r_prev_best, f_r_cur_best, p_ja_best, p_jb_best

    # Scalar Weighted Point Cloud Alignment
    # New function Added to Starter Code
    def align_points(self, p_ja, p_jb):
        p_a = np.mean(p_ja, axis = 1).reshape(3,1)
        p_b = np.mean(p_jb, axis = 1).reshape(3,1)
        w = p_ja.shape[1]
        W_ba = (1/w)*(p_jb - p_b)@(p_ja - p_a).T
