...
%Additional Parameters (Added For Assignment)
xs = linspace((ogxmin+ogres/2),(ogxmax-ogres/2),ognx);
ys = linspace((ogymin+ogres/2),(ogymax-ogres/2),ogny);
ogxs = repmat(xs,ogny,1);
ogys = repmat(reshape(flip(ys),ogny,1),1,ognx);

maxangledisp = (angles(2) - angles(1))/2;
maxdisp = ogres*sqrt(2)/2;
...
%     [minvalx,closestidxx] = min(abs(xs-x_interp(i)));
%     currentx = xs(closestidxx);
%     [minvaly,closestidxy] = min(abs(ys-y_interp(i)));
%     currenty = ys(closestidxy);
    
    x_int = x_interp(i);
    y_int = y_interp(i);

    ogdxs = ogxs-(x_interp(i)*ones(ogny,ognx));
    ogdys = ogys-(y_interp(i)*ones(ogny,ognx));
    ogds = (ogdxs.^2+ogdys.^2).^(1/2);

    ogdthetas = atan2(ogdys,ogdxs);


    newangles = theta_interp(i) + angles;
    newangles = atan2(sin(newangles),cos(newangles));

    %newanglesmin to newanglesmax
    occmask = zeros(ogny,ognx);
    unoccmask = zeros(ogny,ognx);
    rangemask = ogds<y_laser_max;
...
%     axis ij
    ...
    for j=1:npoints
        ...
        angle = newangles(j);
        if angle+maxangledisp > pi
            bound = angle+maxangledisp - (2*pi);
            %angle-maxangledisp to pi and -pi to bound
            anglemask1 = (pi>ogdthetas)&(ogdthetas>(angle-maxangledisp));
            anglemask2 = (bound>ogdthetas)&(ogdthetas>-pi);
            anglemask = anglemask1|anglemask2;
        elseif angle-maxangledisp < -pi
            bound = angle-maxangledisp + (2*pi);
            anglemask1 = (pi>ogdthetas)&(ogdthetas>bound);
            anglemask2 = ((angle+maxangledisp)>ogdthetas)&(ogdthetas>-pi);
            anglemask = anglemask1|anglemask2;
            %bound to pi and -pi to angle+maxangledisp
        else
            anglemask = ((angle+maxangledisp)>ogdthetas)&(ogdthetas>(angle-maxangledisp));
        end

        anglemask = anglemask&rangemask;
        distmask2 = zeros(ogny,ognx);

        if dist<y_laser_max
            distmask = ((dist+maxdisp)>ogds)&(ogds>(dist-maxdisp));
            distmask2 = anglemask&(ogds>(dist+maxdisp));
            occmask = occmask|(anglemask&distmask);
        end
        unoccmask = unoccmask|(anglemask-distmask2);

    end

    unoccmask = unoccmask - occmask;
...
%     axis ij
    ...
    occmask=flip(occmask,1);
    unoccmask=flip(unoccmask,1);
    
    oglo = oglo + (alpha*occmask) + (beta*unoccmask);
    ogp = exp(oglo)./(ones(ogny,ognx)+exp(oglo));
