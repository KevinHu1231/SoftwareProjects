...
ogxs = repmat(linspace(ogxmin+ogres/2,ogxmax-ogres/2,ognx),ogny,1);
ogys = repmat(flip(linspace(ogymin+ogres/2,ogymax-ogres/2,ogny)'),1,ognx);
maxangledisp = (angles(2) - angles(1))*5;
...
for i=2:size(t_laser,1)
    ...
    for n=1:nparticles
        ...
        % Initialize measurements (Added for assignment)
        ogdxs = ogxs - x_particle(n)*ones(ogny,ognx);
        ogdys = ogys - y_particle(n)*ones(ogny,ognx);
        ogds = (ogdxs.^2 + ogdys.^2).^(1/2); 
        ogangles = atan2(ogdys,ogdxs);
        ...
        for beam=1:2         
            ...
            angle = theta_particle(n)+angles(j);
            ...
            if angle+maxangledisp > pi
                ...
                anglemask1 = (pi>ogangles)&(ogangles>(angle-maxangledisp));
                anglemask2 = (bound>ogangles)&(ogangles>-pi);
                anglemask = anglemask1|anglemask2;
            elseif angle-maxangledisp < -pi
                bound = angle-maxangledisp + (2*pi);
                anglemask1 = (pi>ogangles)&(ogangles>bound);
                anglemask2 = ((angle+maxangledisp)>ogangles)&(ogangles>-pi);
                anglemask = anglemask1|anglemask2;
                %bound to pi and -pi to angle+maxangledisp
            else
                anglemask = ((angle+maxangledisp)>ogangles)&(ogangles>(angle-maxangledisp));
            end

            angleoccmask = occmask&anglemask;

            occangledists = (angleoccmask).*ogds;

            occangledists = occangledists(occangledists>0);

            if isempty(occangledists)
                continue
            end
            ylaserpred = min(occangledists,[],"all");
            w_particle(n) = w_particle(n)*w_gain*normpdf(y_laser(i,j), ylaserpred, sqrt(laser_var));
