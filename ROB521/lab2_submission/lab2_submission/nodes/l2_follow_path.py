...
PATH_NAME = 'shortest_path_02_27_08_50.npy'  # saved path from l2_planning.py, should be in the same directory as this file
        ...
        while not rospy.is_shutdown():
            ...
            for opt in range(local_paths_pixels.shape[1]):
                ...
                for timestep in range(local_paths_pixels.shape[0]):
                    ...
                    # print(point,map_shape)
                    rr, cc = disk(point,COLLISION_RADIUS/0.05,shape=map_shape)
                    # map_copy = np.copy(self.map_np)
                    # map_copy[cc,rr] = 1000
                    # print(rr,cc)
                    # print(map_copy[rr,cc])
                    # # print(self.map_np[rr,cc])
                    # plt.imshow(map_copy)
                    # plt.show()
                    if np.any(self.map_np[cc,rr]):
