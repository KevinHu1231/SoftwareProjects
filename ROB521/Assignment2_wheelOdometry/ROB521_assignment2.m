...
for i=2:numodom
    ...
    theta_odom(i) = theta_odom(i-1) + delta_t*omega_odom(i-1);
    if theta_odom(i) > pi
...
for n=1:2
    ...
    for i=1:size(t_laser,1)
        ...
        % Get laser points when angular velocity less than 0.1 rad/s
        if abs(omega_interp(i))<0.1 %Patch 1

            % Laser positions in laser frame
            x_laser_current = cos_angles.*y_laser(i,:);
            y_laser_current = sin_angles.*y_laser(i,:);
    
            % Transformation between laser frame and current robot frame
            dx = -0.1*cos(theta_interp(i)-theta_true(1));
            dy = -0.1*sin(theta_interp(i)-theta_true(1));
            C_cl = [1,0,dx;0,1,dy;0,0,1]; %Patch 2
    
            % Transformation between current robot frame and initial robot frame
            dx = x_interp(i) - x_true(1);
            dy = y_interp(i) - y_true(1);
            dtheta = theta_interp(i) - theta_true(1);
            C_ic = [cos(dtheta),-sin(dtheta),dx;sin(dtheta),cos(dtheta),dy;0,0,1];
    
            % Transform laser points to initial robot frame
            laser_init = C_ic*C_cl*[x_laser_current;y_laser_current;ones(1,npoints)];
            lasers = cat(2,lasers,laser_init);

        end

        % ------end of your point transformation algorithm-------
    end
    lasers = lasers(1:2,2:end);
    scatter(lasers(1,:),lasers(2,:),'filled')
end

xlabel('x [m]');
ylabel('y [m]');
title('map');
legend('Noise','Ground Truth');
