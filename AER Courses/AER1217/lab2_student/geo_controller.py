...
class GeoController():
    ...
    def _compute_desired_force_and_euler(self,
        ...
        k_vel_y = 1
        k_vel_z = 1
        K_pos = np.array([[k_pos_x,0,0],[0,k_pos_y,0],[0,0,k_pos_z]])
        K_vel = np.array([[k_vel_x,0,0],[0,k_vel_y,0],[0,0,k_vel_z]])
        a_fb = -K_pos@(cur_pos-target_pos)-K_vel@(cur_vel-target_vel)
        a_des = a_fb + target_acc + self.grav*np.array([[0],[0],[1]])

        #---------Tip 2: Compute the desired thrust command--------#

        mag_a_des = np.linalg.norm(a_des)
        desired_thrust = self.mass * mag_a_des
        ...
        z_b_des = a_des/mag_a_des
        y_c_x_z_b_des = np.cross(y_c,z_b_des)
        x_b_des = y_c_x_z_b_des/np.linalg.norm(y_c_x_z_b_des)
        y_b_des = np.cross(z_b_des,x_b_des)
        R_des = np.concatenate((x_b_des,y_b_des,z_b_des),axis=-1)
        r = Rotation.from_matrix(R_des)
        desired_euler = r.as_euler('xyz')
