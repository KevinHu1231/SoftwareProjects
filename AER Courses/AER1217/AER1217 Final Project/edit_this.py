...
class Controller():
    ...
    def planning(self, use_firmware, initial_info):
        ...
        waypoints.append((-0.5,  2.0, 2.0))'''
            # center (0,-3,1)
        '''waypoints.append((-0.707,-3.707, 1.0))
        waypoints.append((0.0,   -4.0,   1.0))
        waypoints.append((0.707, -3.707, 1.0))
        waypoints.append((1.0,   -3.0,   1.0))
        waypoints.append((0.707, -2.293, 1.0))
        waypoints.append((0.0,   -2.0,   1.0))
        waypoints.append((-0.707,-2.293, 1.0))
        waypoints.append((-0.99,  -2.99,   1.0))'''

        path_points = ecu.main()

        for i in range(len(path_points)):
            waypoints.append(tuple(path_points[i]))
        '''waypoints.append((0.5, -2.5, 1.0))
        waypoints.append((2.0, -1.5, 1.0))
        waypoints.append((0.0, 0.2, 1.0))
        waypoints.append((-0.25, 1.0, 1.0))
        waypoints.append((-0.5, 1.5, 1.0))'''
        ...
        # RBF network fit
        #fx = Rbf(t, self.waypoints[:,0])
        #fy = Rbf(t, self.waypoints[:,1])
        #fz = Rbf(t, self.waypoints[:,2])

        # CubicSpline fit
        fx = CubicSpline(t, self.waypoints[:,0])
        fy = CubicSpline(t, self.waypoints[:,1])
        fz = CubicSpline(t, self.waypoints[:,2])   


        '''t = np.arange(self.waypoints.shape[0])
        fx = np.poly1d(self.waypoints[:,0])
        fy = np.poly1d(self.waypoints[:,1])
        fz = np.poly1d(self.waypoints[:,2])'''
        duration = 40  # curve fitting segments
    ...
    def cmdFirmware(self,
        ...
        if iteration == 0:
            #print("########## take off ##########")
            height = 1
            duration = 3  # take 0.3s to take off
        ...
        elif iteration >= 3*self.CTRL_FREQ and iteration < 44*self.CTRL_FREQ:
        ...
        elif iteration == 44*self.CTRL_FREQ + 1:
            height = 0.
            duration = 3
