...
class RRT():
    ...
    def __init__(self, start, end, gate_orient, gate, nodes = [], lines = [], obstacles = [], path = None, path_1 = [], path_2 = [], path_lines = [], bias = True, dimension = 3, plot = True):
        ...
        self.end_state_dist = 0.4 # no less than 0.15 allowed
        ...
        self.lower_bound = np.array([-2.5,-2.5,0])
        self.upper_bound = np.array([2.5,2.5,1.3])
...
def interpolate_traj(waypoints,distance):
    ...
    for i in range(len(waypoints) - 1):
        ...
        interpolated_segment = interpolate_segment(point1, point2, distance)
        if i != 0:
...
def interpolate_segment(point1, point2, distance):
    x1 = point1[0]
    y1 = point1[1]
    x2 = point2[0]
    y2 = point2[1]
    ...
    num_points = int(np.round(num_points) + 1)

    xs = np.linspace(x1, x2, num_points)
    ys = np.linspace(y1, y2, num_points)
    return [np.array([xs[i], ys[i], point1[2]]) for i in range(num_points)]
...
def main():
    ...
    gates = [[np.array([0,0.2,1]), False, 3],[np.array([0.5,-2.5,1]), False, 1],[np.array([2,-1.5,1]), True, 2],[np.array([-0.5,1.5,1]), True, 4]]
    ...
    last_waypoint = start
    for i in range(4):
        waypoints_section = get_RRT_waypoints(last_waypoint,points[i+1],orientations[i],gate[i],2)
    ...
    waypoints = interpolate_traj(waypoints,0.2)
