...
def main():
    ...
    for i in tqdm(range(iters)):
        ...
        path_length = path_node_list[-1].cost
        rrt_avg_cost += path_length
    ...
    rrt_avg_cost /= iters
    rrt_star_avg_cost /= iters
    print("RRT Avg Cost: ", rrt_avg_cost)
    print("RRT* Avg Cost: ", rrt_star_avg_cost)

    # creating the dataset
    data = {'RRT':rrt_avg_cost, 'RRT*':rrt_star_avg_cost}
    alg = list(data.keys())
    avg_cost = list(data.values())
    
    fig = plt.figure(figsize = (10, 5))
    
    # creating the bar plot
    plt.bar(alg, avg_cost, color ='blue', 
            width = 0.4)
    
    plt.xlabel("Algorithm")
    plt.ylabel("Average Path Cost")
    plt.title("Average Path Cost of RRT and RRT*")
    #plt.savefig("RRT_RRT*_plot.png")
    plt.show()
