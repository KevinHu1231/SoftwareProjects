...
void qSlicerRos2ModuleWidget::setup()
{
  ...
  // Added for selecting path
  this->connect(d->pathSelectionComboBox, SIGNAL(activated(vtkMRMLNode*)), this, SLOT(onPathSelection(vtkMRMLNode*)));
  this->connect(d->addToPathButton, SIGNAL(clicked(bool)), this, SLOT(onAddToPathButtonPressed()));
  this->connect(d->tracePathButton, SIGNAL(clicked(bool)), this, SLOT(onTracePathButtonPressed()));
...
void qSlicerRos2ModuleWidget::onPathSelection(vtkMRMLNode* markup_node)
{
  vtkSlicerRos2Logic *
    logic = vtkSlicerRos2Logic::SafeDownCast(this->logic());
  if (!logic) {
  ...
  *selected_node = *markup_node;
}

void qSlicerRos2ModuleWidget::onAddToPathButtonPressed()
{
  vtkSlicerRos2Logic *
    logic = vtkSlicerRos2Logic::SafeDownCast(this->logic());
  if (!logic) {
  ...
  path_nodes.push_back(selected_node)
}

void qSlicerRos2ModuleWidget::onTracePathButtonPressed()
{
  vtkSlicerRos2Logic *
    logic = vtkSlicerRos2Logic::SafeDownCast(this->logic());
  if (!logic) {
