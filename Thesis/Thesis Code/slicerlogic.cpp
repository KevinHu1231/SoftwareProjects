...
vtkSlicerRos2Logic::vtkSlicerRos2Logic()
{
  ...
  //Thesis Code Initializer
  mPathPublisher = mNodePointer->create_publisher<sensor_msgs::msg::JointState>("/path_topic", 10);
}
...
void vtkSlicerRos2Logic::TracePath(std::vector<vtkMRMLNode*> path){
  for(auto node : path)
  {
    geometry_msgs::msg::PoseArray path_message;
    path_message.header.stamp = rclcpp::Time::now()
    path_message.frame_id = "Slicer"
    char* nodeName = node->GetClassName();
    double x[3];
    Vector3d normal = tangent.cross(Vector3f::UnitX());

    if (nodeName == "vtkMRMLMarkupsCurveNode") or (nodeName == "vtkMRMLMarkupsClosedCurveNode"){
      vtkPoints* points = node->GetCurvePointsWorld();
      auto n = points->GetNumberOfPoints();

      // Resample points
      node->GetSampledCurvePointsBetweenStartEndPointsWorld(points,sampleDist,0,n-1);
      n = points->GetNumberOfPoints();

      std::vector<Vector3d> spline;
      double x[3];
      for(auto i=0; i<n ;i++)
      {
        points->GetPoint(i,x);
        spline.push_back(Vector3d(x[0], x[1], x[2]));
      }
      Vector3d normal = tangent.cross(Vector3f::UnitX());
      for (int i = 0; i < spline.size() ; ++i) {
        if (nodeName == "vtkMRMLMarkupsCurveNode") && (i == (spline.size() - 1)){
          Vector3d tangent = spline[i] - spline[i-1];
        }
        else if (nodeName == "vtkMRMLMarkupsClosedCurveNode") && (i == (spline.size() - 1)) {
          Vector3d tangent = spline[0] - spline[i];
        }
        else {
          Vector3d tangent = spline[i + 1] - spline[i];
        }
        Quaternionf orientation = calculateOrientation(tangent, normal);

        geometry_msgs::msg::Pose ee;
        ee.position.x = (spline[i])[0];
        ee.position.y = (spline[i])[1];
        ee.position.z = (spline[i])[2];
        ee.orientation.x = orientation.x();
        ee.orientation.y = orientation.y();
        ee.orientation.z = orientation.z();
        ee.orientation.w = orientation.w();
        path_message.poses.push_back(ee);
      }
    }
    else{
      vtkPoints* points;

      if (nodeName == "vtkMRMLMarkupsPlaneNode"){
        node->GetPlaneCornerPointsWorld(points)
      }
      else
      {
        node->GetControlPointPositionsWorld(points)
      }
      auto n = points->GetNumberOfPoints()

      if ((nodeName == "vtkMRMLMarkupsFiducialNode") && (n == 1)){
        point->GetPoint(i,x);
        geometry_msgs::msg::Pose point;
        point.position.x = x[0];
        point.position.y = x[1];
        point.position.z = x[2];
        path_message.poses.push_back(point);
      }
      else{
        std::vector<Vector3d> lines;
        geometry_msgs::msg::Pose ee0;
        geometry_msgs::msg::Pose ee1;

        for(auto i=0; i<n ;i++)
        {
          points->GetPoint(i,x);
          lines.push_back(Vector3d(x[0], x[1], x[2]));
        }

        for (int i=0; i < (2*lines.size()-2) ; ++i) {
          if (i%2 == 0){
            Vector3d tangent = lines[(i/2)+1] - lines[i/2];
            Quaternionf orientation = calculateOrientation(tangent, normal);
            ee0.position.x = (lines[i/2])[0];
            ee0.position.y = (lines[i/2])[1];
            ee0.position.z = (lines[i/2])[2];
            ee0.orientation.x = orientation.x();
            ee0.orientation.y = orientation.y();
            ee0.orientation.z = orientation.z();
            ee0.orientation.w = orientation.w();
            path_message.poses.push_back(ee0);
          }
          else{
            ee1 = ee0;
            ee1.position.x = (lines[((i-1)/2)+1])[0];
            ee1.position.y = (lines[((i-1)/2)+1])[1];
            ee1.position.z = (lines[((i-1)/2)+1])[2];
            path_message.poses.push_back(ee1);
          }
        }
        if (nodeName == "vtkMRMLMarkupsPlaneNode"){
          Vector3d tangent = lines[0] - lines[n-1];
          Quaternionf orientation = calculateOrientation(tangent, normal);
          ee0.position.x = (lines[n-1])[0];
          ee0.position.y = (lines[n-1])[1];
          ee0.position.z = (lines[n-1])[2];
          ee0.orientation.x = orientation.x();
          ee0.orientation.y = orientation.y();
          ee0.orientation.z = orientation.z();
          ee0.orientation.w = orientation.w();
          path_message.poses.push_back(ee0);
          ee1 = ee0;
          ee1.position.x = (lines[0])[0];
          ee1.position.y = (lines[0])[1];
          ee1.position.z = (lines[0])[2];
          path_message.poses.push_back(ee0);
        }
      }
    }
    mPathPublisher->publish(path_message);
  }
}

Quaternionf calculateOrientation(const Vector3f& tangent, const Vector3f& normal) {
  Vector3f x = tangent.normalized();
  Vector3f y = normal.normalized();
  ...
  Quaternionf orientation(rotation_matrix);
  return orientation;
}

// def getCoordinatePoints(self, markupsNode):

//     nodeName = markupsNode.GetClassName()

//     if (nodeName == "vtkMRMLMarkupsCurveNode") or (nodeName == "vtkMRMLMarkupsClosedCurveNode"):
//         points = markupsNode.GetCurvePointsWorld()
//         n = points.GetNumberOfPoints()
//         pts = np.zeros((n,3))
//         data = np.zeros(3)
//         for i in range(n):
...
//         markupsNode.GetSampledCurvePointsBetweenStartEndPointsWorld(points,self.sampleDist,0,n-1)
...
//         if (nodeName == "vtkMRMLMarkupsLineNode") or (nodeName == "vtkMRMLMarkupsAngleNode") or (nodeName == "vtkMRMLMarkupsFiducialNode"):
//             markupsNode.GetControlPointPositionsWorld(points)
//         elif nodeName == "vtkMRMLMarkupsPlaneNode":
//             markupsNode.GetPlaneCornerPointsWorld(points)
...
//         rePts = self.interpolateBetweenPoints(pts[0,:],pts[1,:])
//         for i in range(n-1):
//             intPts = self.interpolateBetweenPoints(pts[i,:],pts[i+1,:])[1:,:]
//             rePts = np.vstack([rePts,intPts])

//         if nodeName == "vtkMRMLMarkupsPlaneNode":
//             intPts = self.interpolateBetweenPoints(pts[n-1,:],pts[0,:])[1:,:]
//             rePts = np.vstack([rePts,intPts])

//     return pts, rePts

// def interpolateBetweenPoints(self,start,end):
//     dists = np.subtract(end,start)
//     dist = np.linalg.norm(dists,2)

//     numSegments = math.ceil(dist/self.sampleDist)
//     ptDist = dists/numSegments
//     intPts = np.zeros((numSegments+1,3))
//     intPts[0,:] = start
//     for i in range(1,numSegments+1):
//         intPts[i,:] = np.add(intPts[i-1,:],ptDist)
//     return intPts