...
class CNN(torch.nn.Module):
    def __init__(self, num_bins): 
        ...
        self.conv1 = torch.nn.Conv2d(3, 16, stride=2, kernel_size=(9,9)) # First convolutional layer with 3 input channels, 16 output channels, stride 2 and 9x9 kernel
        self.pool1 = torch.nn.MaxPool2d((3,3),stride=3) # First 3x3 max pool with stride 3
        self.relu = torch.nn.ReLU() # Non-linear activation function
        self.conv2 = torch.nn.Conv2d(16, 16, stride=1, kernel_size=(9,9)) # Second convolutional layer with 16 input channels, 16 output channels, stride 1 and 9x9 kernel
        self.batch2 = torch.nn.BatchNorm2d(16) # Batch norm layer after second convolution
        self.pool2 = torch.nn.MaxPool2d((2,2),stride=2) # Second 3x3 max pool with stride 2
        self.fc1 = torch.nn.Linear(224,16) # First fully connected layer with 224 inputs and 16 outputs
        self.drop = torch.nn.Dropout(p=0.5) # Drop out after first fully connected layer
        self.fc2 = torch.nn.Linear(16,num_bins) # Second fully connected lauer with 16 inputs and 8 outputs
    ...
    def forward(self, x):
        ...
        x = self.batch2(x) # Batch normalization
        x = self.pool2(x) # Second max pool
        x = self.relu(x) # Non-linear activation function

        x = torch.flatten(x,1) # Flatten input to fully connected layers
...
class dataloader(torch.utils.data.Dataset):
    ...
    def normalize_to_zero_mean(self):
        #---FILL ME IN-----
        mean = np.mean(np.mean(self.images,axis=2),axis=2) # Get the mean pixel value of each image
        self.images = self.images-mean.reshape(-1,3,1,1) # Normalize about the mean of each image so that each image has a mean of zero or sums to zero
        #------------------
        pass
...
if __name__ == "__main__": 
    ...
    dsets = {x: dataloader('{}.mat'.format(x),binsize=binsize, mode=x) for x in ['train', 'val']} 
