...
class CNN(torch.nn.Module):
    def __init__(self, num_bins): 
        ...
        self.conv1 = torch.nn.Conv2d(3, 16, stride=2, kernel_size=(9,9)) # 3 input channels, 16 output channels
        self.conv2 = torch.nn.Conv2d(16, 16, stride=1, kernel_size=(9,9))
        ...
        self.fc1 = torch.nn.Linear(224,16)
        self.fc2 = torch.nn.Linear(16,num_bins)
...
class dataloader(torch.utils.data.Dataset):
    ...
    def __init__(self, matfile, binsize=45, mode='train'):
        ...
        # if self.mode == 'train':
        #     self.train_mean = np.mean(self.images,0)
        #     np.save('train_mean.npy', self.train_mean)
    ...
    def normalize_to_zero_mean(self):
        ...
        #self.images = np.subtract(self.images,self.mean)

        mean = np.mean(np.mean(self.images,axis=2),axis=2)
        self.images = self.images-mean.reshape(-1,3,1,1)
        #------------------
        pass
