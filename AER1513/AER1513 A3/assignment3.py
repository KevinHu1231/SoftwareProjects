...
def batch(k1,k2):
    ...
    possible_values = [1,-1]
    combinations = list(product(possible_values, repeat=6))
    ...
    scores_2 = []
    combos = []
    for index, combination in tqdm(enumerate(combinations, start=1)):
            ...
            for k in range(k2-k1+1):
                ...
                for j in range(rho_i_pj_i.shape[1]):
                    if y_mask[k,j] == True:
                        ...
                        if iteration == 0:
                            epsilon_it_0 = 1e-6*np.array(combination).reshape((6,1))
        ...
        x_diff = x_op[:,0,3] - r_i_vk_i[0,k1:(k2+1)]
        y_diff = x_op[:,1,3] - r_i_vk_i[1,k1:(k2+1)]
        z_diff = x_op[:,2,3] - r_i_vk_i[2,k1:(k2+1)]

        rot_vec = theta_vk_i[:,k1:(k2+1)]
        C_true = Rotation.from_rotvec(rot_vec.T)

        identity = np.eye(3)
        identities = np.repeat(identity[np.newaxis, :], (k2-k1+1), axis = 0)
        theta_diff_crosses = identities - x_op[:,:3,:3]@np.transpose((C_true.as_matrix()),(0,2,1))
        theta_x_diff = theta_diff_crosses[:,1,2]
        theta_y_diff = theta_diff_crosses[:,2,0]
        theta_z_diff = theta_diff_crosses[:,0,1]

        score = np.linalg.norm(x_diff) + np.linalg.norm(y_diff) + np.linalg.norm(z_diff)
        score_2 = np.linalg.norm(theta_x_diff) + np.linalg.norm(theta_y_diff) + np.linalg.norm(theta_z_diff)
        scores.append(score)
    ...
    scores2_np = np.array(scores_2)
    argmin_scores = np.argmin(scores_np)
    argmin_scores_2 = np.argmin(scores2_np)
    best_combination_1 = combos[argmin_scores]
    best_combination_2 = combos[argmin_scores_2]
    print("Best Combination 1: ", best_combination_1)
    print("Best Combination 2: ", best_combination_2)
    print("Scores 1: ", scores_np)
    print("Scores 2: ", scores2_np)
    print("Combos: ", combos)
    #return x_op, cov
