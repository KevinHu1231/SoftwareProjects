...
% Define M using vectorization and avoid arrayfun
M = @(theta) exp(-2 .* integral(psi_2, 0, theta, 'ArrayValued', true));

% Define V_integrand using vectorization and avoid arrayfun
V_integrand = @(theta) psi_1(theta) .* M(theta);

% Compute V using the integral function with 'ArrayValued' option
V = @(theta) -integral(V_integrand, 0, theta, 'ArrayValued', true);

% By passing 'ArrayValued' as true to integral, it should handle vector input

Mminus = M(1);
Vminus = V(1);
delta = dot(sigmaprime_a(0,a,q1tilde),I*sigmaprime_a(1,a,q1tilde)) / (sigmaprime_a(0,a,q1tilde).' * sigmaprime_a(0,a,q1tilde));

Vs = zeros(length(samples));
for i = 1:length(samples)
    th = samples(i);
    q = sigma_a(th,a,qplus(1),q1tilde);
    c_i = epsilon - (B_perp*D(q)*sigmaprime_a(th,a,q1tilde))^2;
    c(i) = c_i;
    Vs(i) = V(th);
end
Vmax = max(Vs);

c(length(samples)+1) = -delta^2/Mminus + e;
c(length(samples)+2) = delta^2/Mminus - 1 + e;
c(length(samples)+3) = ( (Vminus*delta^2) / (Mminus - delta^2) ) + Vmax + e;

if num == 1
    c(length(samples)+4) = I(1,1)*q1tilde/I(1,2) - v1 + e; 
    c(length(samples)+5) = (I(2,1) + 2*I(1,1))*q1tilde / (2*I(1,2) + I(2,2)) - v1;
else
    c(length(samples)+4) = v1 - I(1,1)*q1tilde/I(1,2) + epsilon; 
    c(length(samples)+5) = v1 - (I(2,1) + 2*I(1,1))*q1tilde / (2*I(1,2) + I(2,2));
end
